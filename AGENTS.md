...
- **`.bottom-tabs` mobile uses `bottom: 15px`** — floats above the edge with breathing room. Do NOT set to `0` (touches edge) or to `env(safe-area-inset-bottom)` (creates a white gap between pill and home-indicator on iOS).
